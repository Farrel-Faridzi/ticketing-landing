...
const pillBadgeVariants = cva('inline-flex items-center gap-1.5 rounded-full px-4 py-[7px] text-[13px] font-bold', {
  variants: {
    tone: {
...
export function PillBadge({ tone, children, className }: PillBadgeProps) {
  return <span className={cn(pillBadgeVariants({ tone }), className)}>{children}</span>;
}
