...
export function FaqAccordion() {
  ...
  return (
    <div id="faq" className="border-t border-b border-mkt-border-soft bg-bg-alt px-6 py-16 md:px-16 md:py-20">
      <SectionHeading eyebrow="FAQ" title="Yang sering ditanyakan" />
      <div className="mx-auto flex max-w-[700px] flex-col gap-3">
        {faqs.map((faq, index) => {
          const open = openIndex === index;
          return (
            <div key={faq.question} className={cn('rounded-[14px] border bg-mkt-card px-6.5 py-5.5', open ? 'border-[1.5px] border-mkt-accent' : 'border-mkt-border')}>
              <button
                type="button"
                aria-expanded={open}
              ...
              >
                ...
                <ChevronIcon open={open} size={18} className={open ? 'text-mkt-accent' : 'text-text-faint'} />
              </button>
              {open && <div className="mt-3 text-[14px] leading-[1.6] text-text-muted">{faq.answer}</div>}
            </div>
          );
        })}
      </div>
